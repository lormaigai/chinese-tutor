...
5. Add a property named `OWNER_TOKEN` with a private random value, for example `ct-owner-2026-long-secret`.
6. Click `Deploy > New deployment`.
7. Choose `Web app`.
8. Set `Execute as` to `Me`.
9. Set `Who has access` to `Anyone`.
10. Deploy and copy the Web app URL ending in `/exec`.
11. Paste that URL into `profile-lead-config.js` as `profileLeadEndpoint`.
12. Commit and push the app again.
